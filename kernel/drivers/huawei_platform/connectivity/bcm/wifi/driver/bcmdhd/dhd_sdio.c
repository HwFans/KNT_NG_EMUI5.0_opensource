...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	if (on) {
		...
		sdio_quality_add(CMD_SLEEPCSR_SET, CMD_WRITE, 0);
#endif
...
#ifdef HW_SDIO_QUALITY_STATISTIC
		sdio_quality_add(CMD_SLEEPCSR_CLR, CMD_WRITE, 0);
#endif
...
#ifdef HW_SDIO_QUALITY_STATISTIC
		if (on)
			sdio_quality_add(CMD_SLEEPCSR_SET, CMD_WRITE, 1);
		else
			sdio_quality_add(CMD_SLEEPCSR_CLR, CMD_WRITE, 1);
#endif
	...
	if (err)
	{
		...
		set_wlanfty_status(WLANFTY_STATUS_KSO_ERROR);
#endif
...
dhdsdio_hostmail(dhd_bus_t *bus)
{
	...
	if (hmb_data & HMB_DATA_FWHALT) {
		...
		bus->dhd->busstate = DHD_BUS_DOWN;
#ifdef CONFIG_HW_WLANFTY_STATUS
		set_wlanfty_status(WLANFTY_STATUS_HALTED);
#endif
	}
#endif /* DHD_DEBUG */
...
{
#ifdef SUPPORT_WT_WIFI_MIMO_CALIBRATE
	char CalDateBuf[128*6];
#else
	char CalDateBuf[128*2];
#endif /* SUPPORT_WT_WIFI_MIMO_CALIBRATE */
	int CalDateLen = 0;
	...
	CalDateLen = hw_load_Txcalibrate_data(CalDateBuf, sizeof(CalDateBuf));
	CalDateLen += hw_load_Rxcalibrate_data(CalDateBuf+(strlen(CalDateBuf)), 128);
#ifdef SUPPORT_WT_WIFI_MIMO_CALIBRATE
	CalDateLen += hw_load_MimoTxcckcalibrate_data(CalDateBuf+(strlen(CalDateBuf)), 128);
	CalDateLen += hw_load_MimoTxc1calibrate_data(CalDateBuf+(strlen(CalDateBuf)), 128);
	CalDateLen += hw_load_MimoTxbw0calibrate_data(CalDateBuf+(strlen(CalDateBuf)), 128);
	CalDateLen += hw_load_MimoTxbw1calibrate_data(CalDateBuf+(strlen(CalDateBuf)), 128);
#endif /* SUPPORT_WT_WIFI_MIMO_CALIBRATE */
	if (CalDateLen<=0) {
